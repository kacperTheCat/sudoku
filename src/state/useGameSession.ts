...
interface UseGameSessionOptions {
  ...
  /** Global Podpowiedzi setting — owned by useGame, just passed through for useDailyChallenge. */
  colorAssists: boolean;
  /** Whether this session's screen is the one currently on-screen (pauses the timer otherwise). */
  isActive: boolean;
  /** Fired once when a placement makes the puzzle complete, after the shared win fanfare — for
   *  whatever bookkeeping is specific to this save slot (stats vs. daily streak). */
  onComplete: (next: GameState) => void;
}

/**
 * Everything about playing a single Sudoku game slot that's identical
 * between the regular game and the daily challenge: cell selection, digit
 * placement (with all its feedback — pulse/ripple/combo/exhausted-digit/
 * sound/haptic), erase, undo, notes toggling, the Podpowiedzi sweep, and the
 * elapsed-time ticker. What's genuinely different between the two callers
 * (how `game` is initialized/generated/persisted, and what happens on
 * completion) stays in useGame/useDailyChallenge — this hook only takes
 * `game`/`setGame` as already-lifted state and an `onComplete` callback.
 */
export function useGameSession({ game, setGame, colorAssists, isActive, onComplete }: UseGameSessionOptions) {
  ...
  // Elapsed-time ticker, paused once the puzzle is solved or while this
  // session's screen isn't the one on-screen — otherwise time keeps
  // accruing in the background, unfairly inflating completion times.
  useEffect(() => {
  ...
  }, [game === null, game?.isComplete, isActive, setGame]);
  ...
  const setDigit = useCallback(
    ...
    [game, colorAssists, setGame, onComplete],
  );
